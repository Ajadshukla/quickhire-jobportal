...
const Header = () => {
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="text-center max-w-5xl mx-auto px-4 py-10 md:py-20">
        <div className="flex flex-col gap-6">
          <span className="px-4 md:px-5 mx-auto inline-flex justify-center items-center py-2 gap-2 rounded-full bg-white/90 border border-amber-200 text-amber-700 font-semibold shadow-sm qh-chip text-xs md:text-sm">
          ...
          <h2 className="qh-display text-3xl sm:text-4xl md:text-6xl font-black leading-tight text-slate-900 dark:text-slate-100">
          ...
          <div className="flex flex-col sm:flex-row w-full max-w-3xl shadow-xl border border-slate-300/70 dark:border-slate-700/70 px-3 sm:pl-4 sm:pr-2 py-3 sm:py-2 rounded-2xl sm:rounded-full items-stretch sm:items-center gap-3 mx-auto bg-white/95 dark:bg-slate-900/70">
            <Search className="h-5 w-5 text-slate-400 dark:text-slate-500" />
            <input
              type="text"
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Search roles, skills, or companies"
              className="outline-none border-none w-full bg-transparent text-slate-700 dark:text-slate-200 text-sm md:text-base"
            />
            <Button onClick={searchjobHandler} className="w-full sm:w-auto rounded-xl sm:rounded-full bg-slate-900 hover:bg-slate-800 px-5">
              Search
